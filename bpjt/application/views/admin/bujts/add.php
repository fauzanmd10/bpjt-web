...
<?php echo form_open_multipart('admin/bujts/create', array('class'=>'stdform')); ?>
	<div class="tabbable">
	    <ul class="nav nav-tabs buttons-icons">
	    	<li class="active"><a data-toggle="tab" href="#contents">Tambah BUJT</a></li>
	    </ul>
	    	    ...
	    	    <div class="tabbable1">
					...
					<div class="tab-content">
						<div id="indonesia" class="tab-pane active">					    
							<p>
								<label>Nama</label>
								<span class="field"><input type="text" name="name_id" id="name-id" class="input-xxlarge" placeholder="Nama" value="<?php echo (isset($bujt)) ? $bujt['name_id'] : ''; ?>" /></span>
							</p>
							<p>
								<label>Terbitkan :</label>
								<span class="formwrapper">
									<?php if (isset($bujt)) { ?>
									<input type="radio" name="status_id" value='published' <?php echo ($bujt['status_id'] == 'published') ? 'checked' : ''; ?> /> Ya &nbsp; &nbsp;
									<input type="radio" name="status_id" id="status-id" value='draft' <?php echo ($bujt['status_id'] == 'draft') ? 'checked' : ''; ?> /> Tidak  &nbsp; &nbsp;
									<?php } else { ?>
