...
class Regulations extends CI_Controller {
	...
	public function index($type=null) {
		...
		if ($this->input->get('filename', TRUE)) {
			$query_vars['filename'] = $filename = clean_str($this->input->get('filename', TRUE));
		}
		...
		$data = array(
			'content' => $this->load->view('regulations/index-new', array('regulations'=>$regulations,  'newstickers'=>$newstickers, 'newstickers2'=>$newstickers2, 'newstickers3'=>$newstickers3, 'content_type'=>$type, 'query_vars'=>$query_vars, 'popular_articles'=>$this->article->get_popular_articles(3), 'count_regulations'=>$count_regulations, 'page'=>$page, 'total_page'=>$total_page,'titles' => $titles), true),
