...
class Articles2 extends CI_Controller {
				...
				if ($this->input->post('title_id',true) != '') {
					...
					if ($_FILES['image_id']['error'] != UPLOAD_ERR_NO_FILE) {
						...
						if ($mime == 'image/jpeg' || $mime == 'image/jpg' || $mime == 'image/png' ){
							...
							// $new_imagepath = base_url() . 'uploads/images/' . $article_id . '/' . $new_filename;
							$new_imagepath = base_url() . 'front/img/article/' . $new_filename;

							// move_uploaded_file($_FILES['image_id']['tmp_name'], $upload_dir . '/' . $new_filename);
							$destination =  $upload_dir . $new_filename;
							$source = $_FILES['image_id']['tmp_name'];
							$imgsize= $this->compressImage($source, $destination,95);
							/* image processing */
