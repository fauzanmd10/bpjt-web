...
class Contents extends CI_Controller {
	...
	public function form($content_type, $sub_content_type=null, $filled_value=null) {
		...
		$data = array(
			...
			'content' => $this->load->view('admin/contents/form', array('content'=>$content_data, 'menu'=>$menu, 'menu_name'=>$menu_name, 'en_menu_name'=>$en_menu_name, 'content_type'=>$content_type, 'sub_content_type'=>$sub_content_type, 'flash_success'=>$this->session->flashdata('article_content_success')), true),
			'jscripts' => $jscripts,
	...
	public function save() {
		...
		if ($this->input->post()) {
			...
			if($this->input->post('source',true) != 'BPJT'){
				die();
			}
				...
				if ($saveret) {
					...
					if ($_FILES['image_id']['error'] != UPLOAD_ERR_NO_FILE) {
						...
						if ($mime == 'image/jpeg' || $mime == 'image/jpg' || $mime == 'image/png' ){
							...
							$upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/images/' . $content_id;
							...
							$new_imagepath = base_url() . 'bpjt_ci3/uploads/contents/images/' . $content_id . '/' . $new_filename;
					...
					if ($this->input->post('title_en',true) != "") {
						...
						/* video url */
						// if ($_FILES['video_en']['error'] != UPLOAD_ERR_NO_FILE) {
						// 	$mime = mime_content_type($_FILES['video_en']['tmp_name']);
						// 	if ($mime == 'image/jpeg' || $mime == 'image/jpg' || $mime == 'image/png' ){
						// 		$filename = $_FILES['video_en']['name'];
						// 		$extensions = explode('.', $filename);
						// 		$extension = $extensions[count($extensions)-1];
						// 		$filetype = $_FILES['video_en']['type'];
						// 		$upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/videos/' . $en_content_id;
						// 		$thumb_upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/videos/' . $en_content_id . '/thumb';

						// 		if (!is_dir($thumb_upload_dir)) {
						// 			mkdir($thumb_upload_dir, 0777, true);
						// 		}

						// 		$new_filename = md5($filename . date('YmdHis'));
						// 		$new_filename_with_ext = $new_filename . '.' . $extension;
						// 		$new_videopath = base_url() . 'uploads/contents/videos/' . $en_content_id . '/' . $new_filename_with_ext;
						// 		$new_thumbpath = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/videos/' . $en_content_id . '/thumb/' . $new_filename . '.jpg';

						// 		move_uploaded_file($_FILES['video_en']['tmp_name'], $upload_dir . '/' . $new_filename_with_ext);
						// 		exec($ffmpeg . " -i " . $upload_dir . '/' . $new_filename_with_ext . " " . $options . " " . $new_thumbpath, $output);

						// 		if (!empty($content)) {
						// 			$this->content->remove_meta($en_content_id, 'video');
						// 		}
						// 		$data_meta = array(
						// 			'content_id'=>$en_content_id,
						// 			'meta_key'=>'video',
						// 			'meta_value'=>$new_videopath
						// 		);
						// 		$this->content->insert_meta($data_meta);
						// 	}
						// }
						...
						if ($_FILES['image_en']['error'] != UPLOAD_ERR_NO_FILE) {
							...
							if ($mime == 'image/jpeg' || $mime == 'image/jpg' || $mime == 'image/png' ){
								...
								$upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/images/' . $en_content_id;
								...
								$new_filename = md5($filename . date('YmdHis')) . '.' . $extension;
								$new_imagepath = base_url() . 'uploads/contents/images/' . $en_content_id . '/' . $new_filename;
						...
						/* file uploaded */
						// if ($_FILES['file_en']['error'] != UPLOAD_ERR_NO_FILE) {
						...
						// 		$upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/bpjt_ci3/uploads/contents/files/' . $en_content_id;
						...
						// 		$new_filename = md5($filename . date('YmdHis')) . '.' . $extension;
						// 		$new_filepath = base_url() . 'uploads/contents/files/' . $en_content_id . '/' . $new_filename;

						// 		move_uploaded_file($_FILES['file_en']['tmp_name'], $upload_dir . '/' . $new_filename);

						// 		if (!empty($content)) {
						// 			$this->content->remove_meta($en_content_id, 'file');
						// 		}
						// 		$data_meta = array(
						// 			'content_id'=>$en_content_id,
						// 			'meta_key'=>'file',
						// 			'meta_value'=>$new_filepath
						// 		);
						// 		$this->content->insert_meta($data_meta);
						// 	}
						// }
