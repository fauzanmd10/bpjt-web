jQuery(document).ready(function(){
    // dynamic table
    var inventories_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/inventories/api_get_all_inventories'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_inventories = new Array()
            , selected_rows = inventories_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_inventories.push(key);
            }

            window.location.href = "/admin/inventories/edit/" + checked_inventories[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/inventories/show/" + checked_inventories[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/inventories/destroy",
                            data:{
                                inventory_ids:checked_inventories,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        inventories_table.fnDeleteRow(selected_row);
                                    }
