jQuery(document).ready(function(){
    // dynamic table
    var measurements_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/measurements/api_get_all_measurements'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_measurements = new Array()
            , selected_rows = measurements_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_measurements.push(key);
            }

            window.location.href = "/admin/measurements/edit/" + checked_measurements[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/measurements/show/" + checked_measurements[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/measurements/destroy",
                            data:{
                                measurement_ids:checked_measurements,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        measurements_table.fnDeleteRow(selected_row);
                                    }
