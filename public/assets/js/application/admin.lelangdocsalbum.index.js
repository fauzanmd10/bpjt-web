...
jQuery(document).ready(function(){
    // dynamic table
    var albums_table = jQuery('#dyntablealbum').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/lelangdoc/api_get_all_docalbums?id_ruas=' + jQuery('#id_ruas').val(),
...
jQuery(window).load(function(){
    jQuery('#medialist2').isotope({
        itemSelector : 'li',
        layoutMode : 'fitRows'
    });
    
    // Media Filter
    jQuery('#mediafilter2 a').click(function(){
    
        var filter = (jQuery(this).attr('href') != 'all')? '.'+jQuery(this).attr('href') : '*';
        jQuery('#medialist2').isotope({ filter: filter });
    
        jQuery('#mediafilter2 li').removeClass('current');
