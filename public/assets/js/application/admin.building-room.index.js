jQuery(document).ready(function(){
    // dynamic table
    var building_rooms_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/building_rooms/api_get_all_building_rooms'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_building_rooms = new Array()
            , selected_rows = building_rooms_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_building_rooms.push(key);
            }

            window.location.href = "/admin/building_rooms/edit/" + checked_building_rooms[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/building_rooms/show/" + checked_building_rooms[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/building_rooms/destroy",
                            data:{
                                building_room_ids:checked_building_rooms,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        building_rooms_table.fnDeleteRow(selected_row);
                                    }
