jQuery(document).ready(function(){
    // dynamic table
    var instances_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/instances/api_get_all_instances'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_instances = new Array()
            , selected_rows = instances_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_instances.push(key);
            }

            window.location.href = "/admin/instances/edit/" + checked_instances[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/instances/show/" + checked_instances[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/instances/destroy",
                            data:{
                                instance_ids:checked_instances,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        instances_table.fnDeleteRow(selected_row);
                                    }
