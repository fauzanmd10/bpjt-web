jQuery(document).ready(function(){
    // dynamic table
    var announcements_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/announcements/api_get_all_announcements'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_announcements = new Array()
            , selected_rows = announcements_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_announcements.push(key);
            }

            window.location.href = "/admin/announcements/edit/" + checked_announcements[0];
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/announcements/destroy",
                            data:{
                                announcement_ids:checked_announcements,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        announcements_table.fnDeleteRow(selected_row);
                                    }
