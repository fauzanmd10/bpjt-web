jQuery(function() {
	// dynamic table
    var articles_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/index.php/admin/articles/api_get_all_articles'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_articles = new Array()
            , selected_rows = articles_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_articles.push(key);
            }

            window.location.href = "./articles/edit/" + checked_articles[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "./articles/show/" + checked_categories[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            ...
                            data:{
                                article_ids:checked_articles,
                                bpjt_token:jQuery("#token-value").val()
                            },
                            dataType:"json",
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        articles_table.fnDeleteRow(selected_row);
                                    }
