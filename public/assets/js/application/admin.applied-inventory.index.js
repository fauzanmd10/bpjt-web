jQuery(document).ready(function(){
    // dynamic table
    var applied_inventories_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/applied_inventories/api_get_all_applied_inventories'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_applied_inventories = new Array()
            , selected_rows = applied_inventories_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_applied_inventories.push(key);
            }

            window.location.href = "/admin/applied_inventories/edit/" + checked_applied_inventories[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/applied_inventories/show/" + checked_applied_inventories[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/applied_inventories/destroy",
                            data:{
                                applied_inventory_ids:checked_applied_inventories,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        applied_inventories_table.fnDeleteRow(selected_row);
                                    }
