jQuery(document).ready(function(){
    // dynamic table
    var banner_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/banner/api_get_all_banner'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            , selected_rows = banner_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_categories.push(key);
            }

            window.location.href = "/admin/banner/edit/" + checked_categories[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/banner/show/" + checked_categories[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin mengedit urutan item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/banner/update_sequence",
                            data:{
                            	banner_ids:checked_categories,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        banner_table.fnDeleteRow(selected_row);
                                    }
