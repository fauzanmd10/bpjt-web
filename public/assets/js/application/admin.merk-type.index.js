jQuery(document).ready(function(){
    // dynamic table
    var merk_types_table = jQuery('#dyntable').dataTable({
        ...
        "bProcessing": true,
        "sAjaxSource": '/admin/merk_types/api_get_all_merk_types'
    });
    ...
    $btn_edit.on('click', function(e) {
        ...
        if (checked_count == 1) {
            var checked_merk_types = new Array()
            , selected_rows = merk_types_table.find('tr.row_selected');
            
            for (var key in checked_group) {
                checked_merk_types.push(key);
            }

            window.location.href = "/admin/merk_types/edit/" + checked_merk_types[0];
    ...
    $btn_show.on('click', function(e) {
        ...
        if (checked_count == 1) {
            ...
            window.location.href = "/admin/merk_types/show/" + checked_merk_types[0];
        } else if (checked_count == 0) {
            jAlert('Silakan pilih minimal 1 data terlebih dahulu.');
        } else {
            jAlert('Maksimal memilih 1 data untuk diubah.');
            ...
            if (checked_count == 0) {
                ...
                jConfirm('Anda yakin ingin menghapus item terpilih?', 'Pertanyaan', function(r) {
                    if (r) {
                        ...
                        jQuery.ajax({
                            type:"POST",
                            url:"/admin/merk_types/destroy",
                            data:{
                                merk_type_ids:checked_merk_types,
                            ...
                            success:function(response) {
                                if (response.status == "success") {
                                    for(var i=0; i<selected_rows.length; i++) {
                                        ...
                                        merk_types_table.fnDeleteRow(selected_row);
                                    }
